-- Scoring update: wrong answers now DEDUCT round_number*10 (mirroring the base
-- reward), the running total is floored at 0 (can't go negative), and no time
-- component applies to the penalty. Right answers unchanged: base + time bonus.
...
begin
  ...
  update public.player_game_stats set total_score = greatest(total_score + v_points, 0)
