-- payout_game now computes the Wallet Efficiency Rating (WER) for every player
-- at game end and uses it as the tie-breaker: same score -> higher efficiency
-- (more points per cent spent, faster) wins. Tie detection for Sudden Death
-- also respects WER, so overtime only triggers on a true dead heat (equal
-- score AND equal efficiency).
...
begin
  ...
  -- Compute WER for all players in this game (higher = better).
  update public.player_game_stats
  set weighted_efficiency_score =
    total_score::numeric / greatest(total_cash_spent_cents, 1) / greatest(total_response_time_ms, 1)
  where game_id = p_game_id;

  -- Tie detection now ranks by (score, WER) so a WER difference resolves what
  -- would otherwise look like a tie - only a true dead heat opens Sudden Death.
  select array_agg(distinct rnk) into v_tied_ranks
  from (
    select user_id, rank() over (order by total_score desc, weighted_efficiency_score desc) as rnk
  ...
  for v_winner in
    ...
    where game_id = p_game_id and is_eligible_for_grand_prize = true and is_eliminated = false
    order by total_score desc, weighted_efficiency_score desc limit 3
  loop
    v_place := v_place + 1;
    v_share := case
