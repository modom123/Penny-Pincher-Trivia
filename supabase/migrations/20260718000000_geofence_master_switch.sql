-- Master on/off switch for geo-fencing, on top of the existing allow/deny state
-- lists (20260707011000_geofence_state_allowlist.sql). Turning this off removes
-- location verification entirely (no LOCATION_REQUIRED, no allow/blocklist check)
-- -- useful for local/soft-launch testing without touching the state lists
-- themselves. Defaults to enabled so existing production behaviour is unchanged.
...
-- buy_round: same as 20260707011000, but the whole region check (including the
-- LOCATION_REQUIRED gate) is skipped while geofence_enabled is false.
...
-- my_compliance_status: same as 20260707011000, plus geofenceEnabled in the
-- response, and regionBlocked forced to false while it's off.
