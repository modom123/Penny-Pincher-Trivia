...
--
-- Written idempotently (add column if not exists / create or replace) because
-- an earlier form of this migration may already have been applied to the live
-- database out-of-band; re-running it must be a no-op, not an error.

alter table public.games
  add column if not exists engine_lease_owner text;
alter table public.games
  add column if not exists engine_lease_expires_at timestamptz;

-- Partial index: the watcher only ever scans not-yet-finished games.
create index if not exists idx_games_engine_runnable on public.games (status)
...
-- flags), so "who's winning" mid-game matches who payout_game would pay. Keeps
-- the username in each entry (added by the username gate) and all other timing,
-- prize-pool, and final-round semantics unchanged.
...
begin
  ...
  from (
    ...
    where pgs.game_id = p_game_id
      and pgs.is_eligible_for_grand_prize = true
      and pgs.is_eliminated = false
    order by pgs.total_score desc limit 10
  ) top;
